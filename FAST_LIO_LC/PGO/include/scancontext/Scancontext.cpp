...
// namespace SC2
// {
constexpr double LIDAR_HEIGHT = 2.0; // lidar height : add this for simply directly using lidar scan in the lidar local coord (not robot base coord) / if you use robot-coord-transformed lidar scans, just set this as 0.

constexpr int    PC_NUM_RING = 20; // 20 in the original paper (IROS 18)
constexpr int    PC_NUM_SECTOR = 60; // 60 in the original paper (IROS 18)
constexpr double PC_UNIT_SECTORANGLE = 360.0 / double(PC_NUM_SECTOR);

// tree
 // simply just keyframe gap (related with loopClosureFrequency in yaml), but node position distance-based exclusion is ok.
constexpr int    NUM_CANDIDATES_FROM_TREE = 3; // 10 is enough. (refer the IROS 18 paper)

// loop thres
constexpr double SEARCH_RATIO = 0.1; // for fast comparison, no Brute-force, but search 10 % is okay. // not was in the original conf paper, but improved ver.
...
// config
constexpr int    TREE_MAKING_PERIOD_ = 30; // i.e., remaking tree frequency, to avoid non-mandatory every remaking, to save time cost / in the LeGO-LOAM integration, it is synchronized with the loop detection callback (which is 1Hz) so it means the tree is updated evrey 10 sec. But you can use the smaller value because it is enough fast ~ 5-50ms wrt N.
